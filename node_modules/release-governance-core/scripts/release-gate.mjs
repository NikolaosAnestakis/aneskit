...
const gateMode = (process.env.GATE_MODE || "strict").toLowerCase();
const relaxedMode = gateMode === "relaxed";

const projectRoot = path.resolve(process.env.PROJECT_ROOT || process.cwd());
const distPath = process.env.DIST_PATH || "dist";
const baselineFilePath = process.env.BASELINE_FILE_PATH || "examples/baseline.html";
...
function fileExists(relPath) {
  return fs.existsSync(resolveProjectPath(relPath));
}
...
const cssRel = path.posix.join(distPath.replace(/\\/g, "/"), "aneskit.css");
const minRel = path.posix.join(distPath.replace(/\\/g, "/"), "aneskit.min.css");

if (!fileExists(cssRel) || !fileExists(minRel)) {
  violations.push("Build artifacts missing. Run npm run build before release gate.");
}

if (!fileExists(baselineFilePath)) {
  violations.push(`Baseline file missing: ${baselineFilePath}`);
}
...
if (baselineTag && fileExists(cssRel) && fileExists(minRel)) {
  const oldCss = readTagFile(baselineTag, cssRel);
  const oldMin = readTagFile(baselineTag, minRel);
  const newCss = readText(cssRel);
  const newMin = readText(minRel);
  ...
  if (oldCss === null || oldMin === null) {
    ...
    if (addedSelectors.length === 0 && removedSelectors.length === 0 && changedSelectors.length === 0) {
      classification = "NONE";
    } else if (removedClassSelectors.length > 0) {
      classification = "BREAKING";
    } else if (changedClassSelectors.length > 0) {
      classification = relaxedMode ? "SAFE" : "BREAKING";
    } else if (removedSelectors.length > 0 || changedSelectors.length > 0) {
      classification = relaxedMode ? "SAFE" : "RISKY";
    } else {
      classification = "SAFE";
    }
    ...
    if (!relaxedMode && changedClassSelectors.length > 0 && versionChange !== "major") {
    ...
    if (classification === "SAFE" && !["major", "minor", "patch"].includes(versionChange) && versionChange !== "none") {
      violations.push(`SAFE changes require PATCH, MINOR, or MAJOR bump (detected ${versionChange.toUpperCase()}).`);
    }
    if (classification === "RISKY" && !relaxedMode) {
...
console.log("DEBUG gateMode:", gateMode);
